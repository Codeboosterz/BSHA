  ...
  function initAnimations() {
    ...
    setTimeout(() => {
      const criticalSelectors = [
        '.hero-content h1', '.hero-subtitle', '.hero-buttons .btn',
        '.hero-image-wrapper', '.nav-cta', '.logo',
        '.quick-info-card', '.welkom-image', '.welkom-content',
        '.value-card', '.icon-item', '.gallery-item',
        '.cta-section h2', '.cta-section p', '.cta-section .btn',
        '.footer-grid > div', '.footer-bottom',
        '.page-hero h1', '.page-hero p',
        '.founder-image', '.founder-content h2', '.founder-content p',
        '.founder-content blockquote',
        '.offer-grid .card', '.practical-grid .card',
    ...
    if (quickInfoCards.length) {
      gsap.fromTo(quickInfoCards,
        { y: 40, opacity: 0 },
        {
          ...
          scrollTrigger: { trigger: '.quick-info', start: 'top 85%', toggleActions: 'play none none none' }
        }
      );
    }
    ...
    if (quickInfoGrid) {
      gsap.matchMedia().add('(max-width: 767px)', () => {
        const getTravel = () => {
          const container = quickInfoGrid.closest('.container') || quickInfoGrid.parentElement;
          const visibleWidth = container ? container.clientWidth : window.innerWidth;
          return Math.min(0, visibleWidth - quickInfoGrid.scrollWidth);
        };

        const tween = gsap.to(quickInfoGrid, {
          x: getTravel,
          ease: 'none',
          scrollTrigger: {
            trigger: '.quick-info',
        ...
        return () => {
          tween.kill();
          gsap.set(quickInfoGrid, { clearProps: 'transform' });
        };
      });
    }
    ...
    if (welkomImage && welkomContent) {
      const welkomTrigger = welkomImage.closest('.welkom') || welkomImage.closest('section') || welkomImage;
      const welkomTL = gsap.timeline({
        scrollTrigger: { trigger: welkomTrigger, start: 'top 78%', toggleActions: 'play none none none' }
      });
    ...
    if (valuesHeader) {
      gsap.fromTo(valuesHeader,
        { y: 30, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.7,
          scrollTrigger: { trigger: '#values', start: 'top 82%', toggleActions: 'play none none none' }
        }
      );
    }
    ...
    if (valueCards.length) {
      gsap.fromTo(valueCards,
        ...
        { y: 0, opacity: 1, scale: 1, duration: 0.6, stagger: 0.1, ease: 'back.out(1.2)',
          scrollTrigger: { trigger: '.values-grid', start: 'top 82%', toggleActions: 'play none none none' }
        }
      );
    }
    ...
    if (iconItems.length) {
      gsap.fromTo(iconItems,
        ...
        { y: 0, opacity: 1, scale: 1, duration: 0.5, stagger: 0.08, ease: 'back.out(1.7)',
          scrollTrigger: { trigger: '.icon-strip', start: 'top 88%', toggleActions: 'play none none none' }
        }
      );
    }
    ...
    if (founderInner) {
      const founderTL = gsap.timeline({
        scrollTrigger: { trigger: '.founder-inner', start: 'top 78%', toggleActions: 'play none none none' }
      });
    ...
    // 11. OFFER CARDS
    // =============================================
    const aanbodMotionGrid = document.querySelector('.aanbod-motion-grid');
    if (aanbodMotionGrid) {
      const aanbodCards = gsap.utils.toArray('.aanbod-motion-grid .offer-card');
      if (aanbodCards.length) {
        const mm = gsap.matchMedia();

        mm.add('(min-width: 1100px)', () => {
          const offsets = [
            { xPercent: -18, y: 38, rotate: -1.4 },
            { xPercent: -10, y: -20, rotate: -0.8 },
            { xPercent: -5, y: 24, rotate: -0.35 },
            { xPercent: 5, y: -20, rotate: 0.35 },
            { xPercent: 10, y: 24, rotate: 0.8 },
            { xPercent: 18, y: 38, rotate: 1.4 }
          ];

          aanbodCards.forEach((card, index) => {
            gsap.set(card, {
              xPercent: offsets[index]?.xPercent || 0,
              y: offsets[index]?.y || 0,
              rotate: offsets[index]?.rotate || 0,
              opacity: 1,
              transformOrigin: '50% 65%'
            });
          });

          const tween = gsap.to(aanbodCards, {
            xPercent: 0,
            y: 0,
            rotate: 0,
            ease: 'power2.out',
          ...
          return () => {
            tween.kill();
            gsap.set(aanbodCards, { clearProps: 'transform,transformOrigin' });
          };
        });

        mm.add('(max-width: 1099px)', () => {
          const tween = gsap.fromTo(aanbodCards,
            { y: 36, opacity: 0 },
            { y: 0, opacity: 1, duration: 0.55, stagger: 0.08, ease: 'power2.out',
              scrollTrigger: { trigger: aanbodMotionGrid, start: 'top 82%', toggleActions: 'play none none none' }
            }
          );
    ...
    const offerCards = document.querySelectorAll('.offer-grid:not(.aanbod-motion-grid) .card');
    if (offerCards.length) {
      gsap.fromTo(offerCards,
        { y: 40, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.6, stagger: 0.1, ease: 'power2.out',
          scrollTrigger: { trigger: '.offer-grid', start: 'top 82%', toggleActions: 'play none none none' }
        }
      );
    }
    ...
    if (practicalCards.length) {
      gsap.fromTo(practicalCards,
        ...
        { y: 0, opacity: 1, scale: 1, duration: 0.5, stagger: 0.1, ease: 'back.out(1.2)',
          scrollTrigger: { trigger: '.practical-grid', start: 'top 82%', toggleActions: 'play none none none' }
        }
      );
    }
    ...
    if (contactFormWrap) {
      gsap.fromTo(contactFormWrap,
        ...
        { x: 0, opacity: 1, duration: 0.7, ease: 'power3.out',
          scrollTrigger: { trigger: '.contact-inner', start: 'top 82%', toggleActions: 'play none none none' }
        }
      );
    }
      ...
      if (regCard) {
        gsap.fromTo(regCard,
          ...
          { y: 0, opacity: 1, scale: 1, duration: 0.8, ease: 'power3.out',
            scrollTrigger: { trigger: regCard, start: 'top 88%', toggleActions: 'play none none none' }
          }
        );
      }
    }
